...
--
-- RE-VERIFIED 2026-08-29, AND PARTS 5 AND 7 HAVE SINCE BEEN APPLIED. Probing
-- as `anon`: cookie_balance(), cookie_daily_base() and cookie_daily_ceiling()
-- all answer 42501 "permission denied", which is what an EXISTING function
-- revoked from anon answers - a missing one answers 404 / PGRST202. Since
-- cookie_daily_ceiling() exists only in PART 7, both cookie parts are live.
-- Re-running them is still harmless (every statement is CREATE OR REPLACE), so
-- this file is still safe to paste whole; it is simply no longer true that
-- every part below is outstanding.
--
-- NOTHING TO DO AFTERWARDS - WITH ONE EXCEPTION, AND IT IS PART 8. Every part
-- of this file is DETECTED by the app rather than gated on a hand-flipped
-- constant, so each feature lights up on the next page load: no code change, no
-- flag to flip. PART 8 is the exception, and not because the app fails to
-- detect it. It changes the daily allowance, which the app does pick up on its
-- own - but it is HALF of a change whose other half is a price list that lives
-- in the Edge Functions and does not deploy itself. Read PART 8's own header
-- before pasting it.
--
...
-- works and is empty; see the head of part 4 for why. PARTS 5, 7 AND 8 ARE A
-- CHAIN in that order - 5 creates the cookie tables and functions, 7 replaces
-- four of its constants with an earned ladder, 8 replaces three of those again
-- to flatten the ladder to 15. Each of 7 and 8 guards on the one before and
-- RAISES rather than half-applying, so the order cannot be got wrong silently.
-- ═══════════════════════════════════════════════════════════════════════════


-- ###########################################################################
-- PART 1 of 8 - FRIEND SEARCH BY HANDLE PREFIX
-- ###########################################################################
...
-- ###########################################################################
-- PART 2 of 8 - SCHOOL AND THIS-WEEK LEADERBOARDS
-- ###########################################################################
...
-- ###########################################################################
-- PART 3 of 8 - ONCE-IN-A-LIFETIME FIRST-RUN FLAGS
-- ###########################################################################
...
-- ###########################################################################
-- PART 4 of 8 - MAKE "THIS WEEK" A BOARD WITH PEOPLE ON IT
-- ###########################################################################
...
-- ###########################################################################
-- PART 5 of 8 - COOKIES: THE DAILY AI BUDGET
-- ###########################################################################
...
-- ###########################################################################
-- PART 6 of 8 - ADMIN STUDENT LOOKUP
-- ###########################################################################
...
-- ###########################################################################
-- PART 7 of 8 - COOKIES THAT GROW WITH USE
-- ###########################################################################
...
-- ###########################################################################
-- PART 8 of 8 - FIFTEEN A DAY, AND A PRICE LIST THAT HAS TO SHIP WITH IT
-- ###########################################################################
-- source: supabase/migrations/20260829120000_cookie_budget_15.sql
--
-- MUST RUN AFTER PART 7, which creates the three constants this replaces.
-- Flattens the earned ladder (30, +5 every three days, ceiling 60) to a flat
-- 15 a day for every account, by setting the step to 0 and bringing the
-- ceiling down to meet the base. The arithmetic in cookie_earned_base() is
-- untouched; only the constants it reads have moved.
--
-- READ THIS BEFORE PASTING. Unlike every other part of this file, this one is
-- only HALF of its change. The other half is the price list - chat 2 -> 1,
-- a 40-question set 8 -> 4, plans and Last Minute 5 -> 2, Battle Royale 3 -> 2
-- - which lives in src/lib/cookies.ts and, for the three actions that bill
-- server-side, in a second copy inside supabase/functions/{chat,studybody,
-- last-minute}/index.ts. The Edge Functions do not deploy themselves.
--
-- Applying this part WITHOUT deploying those three Edge Functions puts every
-- student on 15 a day while still charging them the old prices: about seven
-- chat messages, which is a wall rather than a budget. If you can only do one
-- of the two right now, deploy the Edge Functions FIRST and paste this
-- second - that order is merely generous for a few minutes, whereas the other
-- order is punitive.
-- ###########################################################################
